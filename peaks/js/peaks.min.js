 ...
 */
define('m/player/waveform/waveform.points',[
	"m/bootstrap",
	"m/player/waveform/waveform.mixins",
	], function (bootstrap, mixins) {
		...
		return function (waveformView, options) {
			...
			var constructPoint = function (pointId, timeStamp, editable, color, labelText) {
				...
				for (var i = 0; i < pointGroups.length; i++) {
					...
					if (editable) {
						pointGroup.marker = new options.pointMarker(true, pointGroup, point, pointHandleDrag, options.pointDblClickHandler, options.pointDragEndHandler);
			...
			this.init = function() {
				bootstrap.pubsub.on("waveform_zoom_displaying", this.updatePoints);
			};
			...
			this.createPoint = function(timeStamp, editable, color, labelText) {
				...
				if(labelText) {
					...
					pointId = "point" + that.points.length;
				}
			...
			this.removePoint = function(pointId) {
				var pointIndex = _.findIndex(that.points, { id: pointId});
				
				if(pointIndex > -1) {
					//Remove pointgroup from oveview
					//Remove pointgroup from zoomview
					//Remove point from waveform.data.points
					//Remove point from this.points
					//Zombie points?
					...
					that.points.splice(pointIndex, 1);
				}
			};
		};
	});
...
define('m/player/waveform/waveform.core',[
  ...
  "m/player/waveform/waveform.points"
  ], function (bootstrap, WaveformData, WaveformOverview, WaveformZoomView, WaveformSegments, WaveformPoints) {
  ...
  return function () {
    return {
      init: function (options, ui) {
        ...
        //ADDED ADAPTER FROM Waveform-data/lib/builders/webaudio.js
        //TODO: Find a way to integrate this more elegantly.
        ...
        xhr.open("GET", that.options.audioBufferLocation, true);
        xhr.responseType = "arraybuffer";

        xhr.addEventListener("load", function onResponse(progressEvent) {
          
          adapter.fromAudioObject(progressEvent.target.response, function onProcessed(waveform){
      ...
      openZoomView: function () {
        ...
        that.points = new WaveformPoints(that, that.options);
        that.points.init();
      },
    ...
    init: function (opts) {
      ...
      if (!opts.audioElement) {
        ...
        api.options = $.extend({
          ...
          pointMarker: mixins.defaultPointMarker(api.options),
        ...
        $(api.options.container).html(JST.main).promise().done(function () {
          ...
          bootstrap.pubsub.on("waveformOverviewReady", function () {
            ...
            if(api.options.points) { //Any initial points to be displayed?
              api.points.addPoint(api.options.points);
            }
          });
    ...
    points: {
      addPoint: function(timeStamp, editable, color, labelText) {
        if (typeof timeStamp == "number") {
          api.waveform.points.createPoint(timeStamp, editable, color, labelText);
        } else if (typeof timeStamp == "object" && timeStamp.length){
          for (var i = 0; i < timeStamp.length; i++) {
            var point = timeStamp[i];
            api.waveform.points.createPoint(point.timeStamp, point.editable, point.color, point.labelText);
          }
        }
      },

      getPoints: function () {
        return api.waveform.points.points;
      },
      removePoint: function(id) {
        api.waveform.points.removePoint(id);
      }
    },
